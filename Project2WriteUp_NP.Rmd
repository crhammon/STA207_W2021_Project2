...
fit1 <- lm(I(afatal/pop*100000) ~ beertax_change + drinkage + breath + jail 
          + service + state + year, 
          data = Fatalities)
...
```{r, echo = FALSE}
fit2 <- lm(I(fatal/pop*100000) ~ beertax_change + drinkage + breath + jail 
          + service + state + year, 
          data = Fatalities)

summary(fit2)$coefficients[1:6,]
  # not reporting states
```
